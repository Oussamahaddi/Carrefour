<template>
    <div class="w-11/12 mx-auto">
        <div class="bg-gradient-to-b from-[#003772] to-[#0057B4] p-4">
            ...
            <div class="my-2">
                ...
                <div class="flex justify-around ">
                    <div @click="selectMethod(method.id)" v-for="method in methods" class="border border-gray-300 rounded-md px-2 cursor-pointer" :title="method.id">
            ...
            <div>
                <form @submit.prevent="orderNow">
                    ...
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="password" name="floating_password" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-red-500 peer" placeholder=" "  />
                    ...
                    <div class="flex justify-center w-full">
                        <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 shadow-[0_4px_10px] shadow-black/20 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center ">Pay Now</button>
                    </div>
                </form>
                <form @submit.prevent="orderNow" class="hidden">
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" id="floating_email" class="block py-2.5 px-0 w-full text-sm text-white bg-transparent border-0 border-b-2 border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-red-500 peer" placeholder=" "  />
                        <label for="floating_email" class="peer-focus:font-medium absolute text-sm text-white duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-white  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Cart holder</label>
                    </div>
...
function selectMethod(method) {
    ...
    console.log(defaultMethod.value);
}
